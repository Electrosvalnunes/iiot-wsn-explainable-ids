...
# ==========================================================
# 4. Discretizar métricas em 5 níveis
...
# Estrutura: Attack_Class -> métricas
...
for feat in features:
    ...
    table.to_csv(f"bn5_cpt_{feat}.csv")
...
def predict_bn(row):
    ...
    # Converter log-probabilidades em probabilidades normalizadas
...
print("\nAccuracy da Rede Bayesiana com 5 bins:", round(acc, 4))
...
# ==========================================================
cm_df.to_csv("bn5_confusion_matrix.csv")

probabilities_df = pd.DataFrame(probability_rows)
probabilities_df.to_csv("bn5_prediction_probabilities.csv", index=False)

edges = [(target, feat) for feat in features]
edges_df = pd.DataFrame(edges, columns=["Parent", "Child"])
edges_df.to_csv("bn5_structure_edges.csv", index=False)

priors.to_csv("bn5_priors.csv", header=["Probability"])
...
for feat, edges_array in zip(features, disc.bin_edges_):
    bin_edges_data.append({
        "Feature": feat,
        "Very_Low_to_Low": edges_array[1],
        "Low_to_Medium": edges_array[2],
        "Medium_to_High": edges_array[3],
        "High_to_Very_High": edges_array[4]
    })

bin_edges_df = pd.DataFrame(bin_edges_data)
bin_edges_df.to_csv("bn5_discretization_thresholds.csv", index=False)

print("\nArquivos salvos com sucesso:")
print("bn5_confusion_matrix.csv")
print("bn5_prediction_probabilities.csv")
print("bn5_structure_edges.csv")
print("bn5_priors.csv")
print("bn5_discretization_thresholds.csv")

for feat in features:
    print(f"bn5_cpt_{feat}.csv")

print("\nRede Bayesiana com 5 bins executada com sucesso!")